      ...
      <el-row class="main" style="height: 100%">
        <el-col :xs="24" :sm="24" :md="24" :lg="16" class="col-item">
          ...
          <GraphContainer title="运行状态" class="graph-item xpanel-wrapper-6">
            <el-button type="info" size="mini" style="float: right;margin-top: -20px;margin-right: 10px"  @click="backList">返回</el-button>
            <ProdlineStatus />
            <ProdlineDetail />
          </GraphContainer>
...
// 业务组件
import PowerLineChart from '../company/PowerLineChart'
import OutputBarChart from '../company/OutputInBarChart'
import ProdlineIconList from '../company/ProdlineIconList'
import ProdlineStatus from '../company/ProdlineStatus'
import OperatingStatusBarChart from '../company/OperatingStatusBarChart'
import MonitorModel from '@/models/monitor'
import ProdlineDetail from '../company/ProdlineDetail'

export default {
  name: 'DataVisualProdLine',
  components: {
    Screenfull,
    GraphContainer,
    PowerLineChart,
  ...
  computed: {
    ...mapState({
      companyId: state => state.company.pipeLine.companyId,
      pipeLine: state => state.company.pipeLine
    })
  },
  created() {
    let id = this.pipeLine.companyId
  ...
  methods: {
    backList() {
      let data = this.pipeLine
      this.$store.commit('company/SER_PIPELINE_ID', data)
      this.$router.push(`/data-visual/company?id=${data.companyId}`)
    },
    goOverviewPage() {
